 ...
 *
 * @param <T>
 *            The element's type that the tree contains
 */
public class ArrayNTree<T extends Comparable<T>> implements NTree<T> {
	...
	private int proposePosition(T elem) {
		...
		for (int i = 0; i < capacity; i++) {
			...
			if (children[i].data.compareTo(elem) > 0) {
				// element should not be place here or ahead, go back one position, and end
				// search
				index--;
				break;
			}
	...
	/**
	 * Is this tree equal to another object? Two NTrees are equal iff they have the
	 * same values
	 */
	@SuppressWarnings("unchecked")
